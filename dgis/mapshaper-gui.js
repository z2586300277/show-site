  ...
  */
  ...
  var ExportControl = function(gui) {
    ...
    function initLayerMenu() {
      ...
      // update menu title
      gui.container.findChild('.export-layers .menu-title').html(layers.length == 1 ? '图层名称' : '图层');
  ...
  function MshpMap(gui) {
    ...
    function getContentLayerBounds() {
      ...
      if (!b.hasBounds()) {
        ...
        // b.setBounds(0,0,0,0);
        b.setBounds(projectLatLonBBox([70,0,140,55], _dynamicCRS));
      }
      return b;
    }
  ...
  function Basemap(gui) {
    ...
    function onUpdate() {
      ...
      if (!dataCRS || !displayCRS || !crsIsUsable(displayCRS) || !crsIsUsable(dataCRS)) {
        ...
        note = `你的数据 ${activeStyle ? '' : '将'} 显示使用墨卡托投影.`;
    ...
    function checkBounds(bbox) {
      ...
      extentNote.html(msg + '去查看底图').show();
      return false;
    }
